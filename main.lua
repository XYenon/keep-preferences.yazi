...
---@param pref KeepPreferencesPref
---@return KeepPreferencesSortForm
local function sort_form(pref)
	return {
		pref.sort_by,
...
---the active tab's recorded value whenever the tab enters a directory.
---@param state table Plugin state provided by `ya.sync`.
---@param pref KeepPreferencesPref? Preference to apply; `nil` is ignored.
local apply_pref = ya.sync(function(state, pref)
	if not pref then
		ya.dbg(PLUGIN, "apply skipped", "pref=nil")
		return
	end

	ya.dbg(PLUGIN, "apply", pref_text(pref))
	local id = tab_id()
	local cwd = cwd_key()
	...
	ya.dbg(PLUGIN, "protect restoring pref", "tab", id, "cwd", cwd, pref_text(pref))

	state[STATE.applying] = true
	rt.mgr.ratio = clone_ratio(pref.ratio)
	ya.emit("sort", sort_form(pref))
	ya.emit("hidden", hidden_form(pref))
	ya.emit("linemode", { pref.linemode })
	ya.emit("app:resize", {})
